...
function initLanding(): void {
  ...
  const podiumSection = document.querySelector<HTMLElement>(".podium-section");
  const podiumEl = document.getElementById("podium");
  if (!spacer || !podiumSection || !podiumEl || prefersReducedMotion()) return;
  ...
  const runLandingSequence = (): void => {
    ...
    if (remaining <= 0) return; // 이미 다 보이면 스크롤할 필요 없음
