...
import { BROADCAST_CHANNEL_NAME, STORAGE_KEYS, isAIRecord, loadLocalRecords, parseRecordLine, recordsToTxt, saveLocalRecords, sortRecords, uid, } from "./common.js";
...
const els = {
    ...
    fsLinkBtn: document.getElementById("fsLinkBtn"),
...
function handleAdd() {
    ...
    lines.forEach((line, i) => {
        ...
        const record = { ...parsed, id: uid(), createdAt: new Date().toISOString() };
        added.push(record);
    });
    if (errors.length > 0) {
...
async function getFileSha(cfg, token, path) {
    ...
    if (res.status === 404)
        return undefined;
    if (!res.ok)
        throw new Error(`sha 조회 실패 (${res.status})`);
...
async function syncToGitHub() {
    ...
    try {
        ...
        els.syncStatus.textContent = `동기화 실패: ${e.message}`;
...
async function writeLocalFile() {
    ...
    try {
        ...
        els.fsStatus.textContent = `로컬 파일 쓰기 실패: ${e.message}`;
    }
}
function initFileSystemLink() {
